...
In questo capitolo verrà prima data una descrizione dettagliata dell'ambiente di test utilizzato e delle diverse configurazioni su cui sono stati eseguiti i programmi Mapreduce e Pregel, verranno descritte successivamente le metriche utilizzate per confrontare le prestazioni dei due modelli e la metodologia utilizzata per raccogliere i dati necessari. Successivamente verranno mostrati i risultati ottenuti e un analisi delle conclusioni scaturite dal confronto fra i due modelli.
...
In questa sezione verranno descritti prima i dataset su cui sono stati eseguiti gli algoritmi implementati sui due modelli. Successivamente sarannò analizzate le metriche utilizzate per la rilevazione ed il confronto delle prestazioni ottenute dagli esperimenti effettuati. Infine verrà descritto le modalità con cui sono stati esegui i test e la raccolta dei dati utili.
...
I dataset sono stati scelti in modo da avere dimensione di input di scala differente. In questo modo oltre ad avere un confronto tra le prestazioni dei diversi modelli, anche un confronto tra come questi modelli scalano su dimensioni di input diverse.
...
Come anticipato nel secondo e terzo capitolo, le implementazioni dei modelli utilizzate sono rispettivamente Apache Hadoop \cite{1_hadoop.apache.org_2015} per Mapreduce e Apache Giraph \cite{4_giraph.apache.org_2015} per Pregel. Essendo Apache Giraph costruito sopra Apache Hadoop, è stato possibile sfruttare il report fornito da quest'ultimo per ricavare i valori relativi alle metriche selezionate.

Il \textit{tempo CPU} è calcolato come la somma totale dei tempi di occupazione della CPU da parte dei processi Mapreduce sul totale delle macchine utilzzate nell'elaborazione. Su ogni macchina l'informazione è ricavata dai dati presenti nel file /proc/cpuinfo del sistema operativo Linux utilizzato da AWS.
...
L'esecuzione su dataset di dimensione differenza, come già visto, ci permette di confrontare come le prestazioni dei due modelli scalino in rapporto alla dimensione del grafo in input.
...
Infine l'intero set di esperimenti è stato eseguito su due diversi insiemi di macchine, uno composto da 8 unità ed uno composto da 16 unità. Questo ci ha permesso di ricavare una stima di come le prestazione dei due modelli scalino in base alla dimensione del cluster su cui sono eseguiti.
...
Infatti è possibile notare che i risultati ottenuti per l'algoritmo \textit{DegreeComputation} rappresentati in Figura \ref{tab:degree8} e \ref{tab:degree16} mostrano un occupazione della CPU meno netto rispetto agli altri algoritmi che effettuano più iterazioni sul grafo come ad esempio avviene in SSSP \ref{tab:SSSP8} \ref{tab:SSSP16} e Denset Subgraph nella versione per grafi orientati \ref{tab:DenesestNO8} \ref{tab:DenesestNO16}.

L'unico caso in cui Mapreduce mostra comportamento migliore è in DegreeComputation eseguito su cluster di 16 macchine sul dataset dimensioni minori, Facebook \ref{tab:degree16}. In questo caso hanno inciso sul confronto tempi di inizializzazione del framework Apache Giraph rispetto a Apache Hadoop. Tempi di inizializzazione che risultano trascurabili rispetto al tempo totale di computazione in tutti gli altri test affrontati sugli altri due grafi di dimensione maggiore. 
...
Anche analizzando i risultati ottenuti sul consumo di memoria si può notare che, all'aumentare del numero di volte che l'algoritmo itera sul grafo, gli algoritmi sviluppati in \textit{Pregel} hanno un consumo minore di memoria in confronto agli stessi algoritmi sviluppati in \textit{Mapreduce}. Questo comportamento si può notare nei risultati relativi agli esperimenti con più iterazioni, per esempio SSSP \ref{tab:GRPHSSSP8} \ref{tab:GRPHSSSP16} o Denesest Subgraph su grafi non orientati, dove viene mostrato il guadagno netto in termini di memoria per le versioni degli algoritmi sviluppati in Pregel. Al contrario, si può notare che i risultati relativi a DegreeComputation \ref{tab:GRPHdegreeMem8} \ref{tab:GRPHdegree16}, il cui algoritmo prevede una singola iterazione sul grafo, il guadagno nell'utilizzo di memoria risulta meno netto nel confronto con gli algoritmi in MapReduce.
...
Lo svantaggio nel mantenere in memoria l'intero stato del grafo è che il cluster deve avere un numero totale di risorse tali da poter supportare l'elaborazione dell'intera computazione in \textit{Pregel}. Mentre \textit{Mapreduce} suddivide l'elaborazione in task che vengono eseguiti in serie dalla macchine che compongono il cluster, dove ogni task può essere elaborato in istanti differenti. In \textit{Pregel} ogni macchina deve avere un numero di risorse tale da sopportare il calcolo di tutta la porzione di sottografo assegnata.
Questo è possibile notarlo, per esempio, negli esperimento eseguiti per l'algoritmo \textit{NodeIterator++} \ref{tab:GRPHNodeIterator8} \ref{tab:GRPHNodeIterator16}. Per questo algoritmo, i cluster utilizzati, non possedevano risorse sufficienti a mantenere l'intera elaborazione del grafo relativo al dataset \textit{LiveJournal}.
