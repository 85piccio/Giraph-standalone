...
\lstinputlisting[style=customc,caption={Pseudocode funzione \textbf{MAP}}]{code/esempioMap.c}
\end{minipage}
...
Inoltre è stata utilizzata l'implementazione dell'algoritmo NodeIterator++ in MapReduce per \textit{Apache Hadoop} \cite{1_hadoop.apache.org_2015} implementata in \cite{DBLP:journals/corr/FinocchiFF14}.
...
L'algoritmo per il calcolo del grado dei nodi, prende in input un Grafo non orientato \textit{G = (V,E)} dove\textit {V} sono i vertici del grafo \textit{G} ed \textit{E} sono gli archi del grafo \textit{G}. Per ogni nodo \textit{$v \epsilon V$} l'algoritmo calcola il grado del vertice \textit{v}, \textit{deg(v)}, \textit{deg(v)=|\{(v,u)|$(v,u) \epsilon E$\}|}.

\subsection{MapReduce}

L'algoritmo \textit{DegreeCalculator} è stato implementato in una singola iterazione \textit{MapReduce}.

La funzione \textit{Map} prende in input la coppia di valori \textit{<v,u>} che rappresenta un arco del grafo e restituisce in output la coppia \textit{<v,u>}.

La funzione \textit{Reduce} riceve in input la coppia di valori \textit{<v, list(u)>} dove \textit{list(u)} è la lista  di vertici collegati da un arco a \textit{v}. La funzione \textit{Reduce} restituisce in output la coppia di valori \textit{<v,degree(v)>} dove \textit{degree(v)} è il grado del nodo \textit{v} dato dal numero di vertici presenti in \textit{list(u)}.

\begin{minipage}{\linewidth}
\lstinputlisting[style=customc,caption={Pseudocodice funzione \textit{Map}}]{code/esempioMap.c}
\end{minipage}
\begin{minipage}{\linewidth}
\lstinputlisting[style=customc,caption={Pseudocodice funzione \textit{Reduce}}]{code/esempioReduce.c}
\end{minipage}
...
L'implementazione \textit{Pregel}  di \textit{DegreeCalculator} implementata è composta da un solo \textit{Superstep}. Il valore associato ad ogni vertice rappresenta il grado del vertice. 

Nel \textit{Superstep} la funzione \textit{Compute} conta gli archi associati alla struttra dati del vertice e lo associa al valore associato al vertice. Finito il primo Superstep tutti i vertici passano allo stato \textit{Vote to Halt}.
...
\section{SSSP, Cammini minimi da singola sorgente}

\textit{Single Source Shortest Path (SSSP)} è l'implementazione per grafi non pesati dell'algoritmo per il calcolo di cammini minimi di Bellman-Ford. Ad ogni vertice del grafo \textit{v} è associato un valore \textit{distanza\ped{v}} inizializzato a $\infty$ tranne per il nodo sorgente il cui valore è inizializzato a 0.

In Figura \ref{fig:SSSP} è rappresentato un esempio di iterazione del algoritmo SSSP su di un grafo. 
Ad ogni iterazione di \textit{SSSP} ogni nodo \textit{v} confronta \textit{distanza\ped{v}} con i valori \textit{distanza\ped{u}} dei nodi \textit{u} vicini del nodo \textit{v}.
Il valore del nodo \textit{v} viene aggiornato nel corso dell'iterazione se \textit{distanza\ped{v}} < \textit{min(distanza\ped{u}) + 1}, con \textit{min(distanza\ped{u})} valore più piccolo tra i valori \textit{distanza\ped{u}}. .

L'algoritmo termina dopo \textit{|V|} iterazioni di \textit{SSSP} oppure, se nel corso di un iterazione, nessun vertice del grafo aggiorna il valore \textit{distanza} associato.


\begin{figure}
\centering
\includegraphics[width=1\textwidth]{SSSP}
\caption{Esempio iterazioni SSSP}
\label{fig:SSSP}
\end{figure}
...
Per implementare SSSP nel modello \textit{MapReduce}, ogni vertice \textit{v} del grafo viene rappresentato nel formato \textit{ID\ped{v}:distanza\ped{v}}. I valori dei vertici \textit{u} del grafo  sono inizializzati a \textit{ID\ped{u}:$\infty$} tranne il nodo \textit{s} sorgente inizializzato a \textit{ID\ped{s}:0 }.

Ogni iterazione di \textit{SSSP} è implementata attraverso 2 iterazioni \textit{MapReduce}. L'algoritmo termina quando non vengono più modificati i valori \textit{distanza} dei nodi, si esce dall'iterazione SSSP e si passa ad un ultima iterazione \textit{MapReduce} che produce in output il risultato finale dell'algoritmo. Il risultato prodotto è nel formato \textit{<ID\_nodo, distanza\ped{minS}>}, dove \textit{distanza\ped{minS}} è la distanza minima dal nodo sorgente.
...
\caption{Esempio delle 2 iterazioni \textit{MapReduce} che implementano un interazione \textit{SSSP}}
\label{fig:MRSSSP}
\end{figure}

In Figura \ref{fig:MRSSSP} è rappresentato un esempio di un iterazione di \textit{SSSP} in \textit{MapReduce}.
La prima iterazione \textit{MapReduce} si occupa di "propagare" il valore \textit{distanza} sugli archi del grafo.
La funzione \textit{Map} prende in input la coppia di valori \textit{<v:distanza\ped{v},u:distanza\ped{u}>} e produce in output la coppia \textit{<v:distanza\ped{v},u:distanza\ped{u}>}.
La funzione \textit{Reduce} riceve in input la coppia di valori \textit{<v:distanza\ped{v}, list(u:distanza\ped{u})}, dove \textit{list(u:distanza\ped{u})} è la lista di nodi \textit{u} raggiungibili dal nodo \textit{v}. \textit{Reduce} produce in output, per ogni elemento $\textit{w} \epsilon \textit{list(u:distanza\ped{u})}$ la coppia di valori <v:distanza\ped{v},u:distanza\ped{min}>, dove il valore \textit{distanza\ped{min}} è calcolato come il valore minore tra  \textit{distanza\ped{i}} ed il minimo tra valori \textit{distanza\ped{u} + 1} presenti in \textit{list(u:distanza\ped{u})}. 

\begin{minipage}{\linewidth}
\lstinputlisting[style=customc,caption={Pseudocodice funzione \textit{Map} prima itarzione \textit{MapReduce} di \textit{SSSP}}]{code/esempioMapSSP1fase.c}		
\end{minipage}

\begin{minipage}{\linewidth}
\lstinputlisting[style=customc,caption={Pseudocodice funzione \textit{Reduce}prima itarzione \textit{MapReduce} di \textit{SSSP}} ]{code/esempioReduceSSP1fase.c}		
\end{minipage}

La seconda iterazione \textit{MapReduce} è necessaria per permettere di aggiornare i valori \textit{distanza} relativi ai  nodi sorgenti degli archi, dopo la prima iterazione sono aggiornati soltanto i nodi destinazione. 

La funzione \textit{Map} della seconda  iterazione prende in input gli archi nel formato \textit{<v:distanza\ped{v},u:distanza\ped{u}>} e produce il doppio output formato dalle coppie \textit{<v, u:distanza\ped{u}:distanza\ped{v}:S>} e \textit{<u ,v:distanza\ped{v}:distanza\ped{u}:D>}, \textit{S} e \textit{D} sono dei valori segnaposto, \textit{S} segnala che il nodo v è un nodo sorgente e \textit{D} che \textit{v} è invece nodo destinazione. I valori sentinella vengono utilizzati nella funzione \textit{Reduce} per ricostruire il corretto verso degli archi del grafo. 
...
Per ogni valore presente nella lista  \textit{<v, list(u:distanza\ped{u}:distanza\ped{v}:\$)} con \$ = \textit{S}, la funzione \textit{Reduce} produce in output la coppia di valori \textit{<v:distanza\_minima\ped{i}: u:distanza\ped{u}>}, dove \textit{distanza\_minima\ped{i}} è il minore dei valori \textit{distanza\ped{i}} in \textit{list(v\ped{j}:distanza\ped{j}:distanza\ped{i}:\$) con \$ = \textit{D}}.

Le iterazioni \textit{SSSP} terminano quando non vengono aggiornati i valori \textit{distanza} nella corso della prima it\textit{MapReduce}.
...
L'ultima iterazionne \textit{MapReduce}, che salva i risultati in output, viene eseguita quando sono terminate le iterazioni \textit{SSSP}.
La funzione \textit{Map} prende in input la coppia di valori \textit{<v:distanza\ped{v}, u:distanza\ped{u}>}  e produce in output la coppia \textit{<v:distanza\ped{v}, $\varepsilon$>}, dove $\varepsilon$ è stringa vuota.

La funzione \textit{Reduce} prende in input la coppia di valori \textit{<v:distanza\ped{v},list($\varepsilon$)>} e produce in output la coppia \textit{<v, distanza\ped{v}>} con \textit{distanza\ped{v}} valore del cammino minimo dal nodo sorgente al nodo \textit{v}.
...
L'implementazione in \textit{Pregel} sviluppata per realizzare l'algoritmo \textit{SSSP} è costituita da un primo \textit{Superstep} dove il valore associato al vertex \textit{v},  che rappresenta il valore \textit{distanza\ped{v}} associato a \textit{v}, viene inizializzato a $\infty$ tranne per il vertice sorgente il cui valore è inizializzato a 0.

Nella Figura \ref{fig:PREGELSSSP} è rappresentato un esempio di computazione in \textit{Pregel} di \textit{SSSP}.

Ad ogni \textit{Superstep S} è eseguita un'iterazioni \textit{SSSP} in cui un vertice \textit{v} riceve i messaggi, inviati nel \textit{Superstep S-1}, messaggi che contengono il valore \textit{distanza\ped{u}} dei vertici \textit{u} vicini di \textit{v}.
Il vertice \textit{v} confronta i messaggi ricevuti con il proprio valore \textit{distanza\ped{v}}. Nel caso che almeno uno dei messaggi contiene un valore \textit{distanza\ped{u}} che risulta minore di \textit{distanza\ped{v}}, il valore di \textit{distanza\ped{v}} viene aggiornato con il valore minore contenuto nei messaggi.
Nel caso il vertice \textit{v} viene aggiornati nel \textit{Superstep S}, viene inviato un messaggio contenente il nuovo valore \textit{distanza\ped{v} + 1} ai vertici vicini di \textit{v}, che verrà recapitato nel \textit{Superstep} successivo \textit{S+1}. Tutti i vertici passano allo stato \textit{Vote to Halt} e solo i vertici a cui è stato inviato un messaggio in S+1 saranno nello stato \textit{Active} in \textit{S+1}.
...
\begin{minipage}{\linewidth}
\lstinputlisting[style=customc,caption={Pseudocodice funzione \textit{Compute} SSSP}]{code/esempioPregeleSSSP2.c}	
\end{minipage}
...
\begin{minipage}{\linewidth}
\lstinputlisting[style=customc,caption={Pseudocodice NodeIterator++}]{code/nodeiterator.c}	
\end{minipage}
...
\begin{minipage}{\linewidth}
\lstinputlisting[style=customc,caption={Pseudocodice Map Fase 1 NodeIterator++}]{code/nodeiteratorMRMapfase1.c}	
\end{minipage}
\begin{minipage}{\linewidth}
\lstinputlisting[style=customc,caption={Pseudocodice Reduce Fase 1  NodeIterator++}]{code/nodeiteratorMRReducefase1.c}	
\end{minipage}
...
\begin{minipage}{\linewidth}
\lstinputlisting[style=customc,caption={Pseudocodice Map Fase 2 NodeIterator++}]{code/nodeiteratorMRMapfase2.c}	
\end{minipage}
\begin{minipage}{\linewidth}
\lstinputlisting[style=customc,caption={Pseudocodice Reduce Fase 2  NodeIterator++}]{code/nodeiteratorMRReducefase2.c}	
\end{minipage}
...
Come nell'esempio in \ref{fig:PREGELNIT}, nel grafo \textit{G'}, dopo il secondo \textit{Superstep S\ped{1}}, rimangono solo gli archi che rispettano l'ordine definito da $\prec$, come ad esempio \textit{(0,1)} e \textit{(1,3)}.
...
\lstinputlisting[style=customc,caption={Pseudocodice NodeIterator++ Pregel}]{code/esempioPregeleNodeIterator.c}	
...
\begin{minipage}{\linewidth}
\lstinputlisting[style=customc,caption={Pseudocodice Denesest Subgraph per grafi non orientati}]{code/densestU.c}
\end{minipage}
...
\begin{minipage}{\linewidth}
\lstinputlisting[style=customc,caption={Pseudocodice Denesest Subgraph per grafi non orientati}]{code/esempioPregeleDensetU.c}
\end{minipage}
...
\begin{minipage}{\linewidth}
\lstinputlisting[style=customc,caption={Pseudocodice Denesest Subgraph per grafi orientati}]{code/densestD.c}
\end{minipage}
