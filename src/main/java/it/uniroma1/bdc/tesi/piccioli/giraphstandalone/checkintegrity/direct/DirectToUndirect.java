 ...
 */
package it.uniroma1.bdc.tesi.piccioli.giraphstandalone.checkintegrity.direct;
...
public class DirectToUndirect extends BasicComputation<IntWritable, DoubleWritable, NullWritable, IntWritable> {
...
//            this.sendMessageToAllEdges(vertex, vertex.getId());
        ...
        if (superstep == 1) {
            ...
            //print verso opposto archi solo se non gia esistenti
            for (IntWritable msg : messages) {
                if (!edgeSet.contains(msg.get()) && msg.get() != vertex.getId().get()) {
                    System.out.println(vertex.getId() + " " + msg.get());
                }
            }
            vertex.voteToHalt();
        }
