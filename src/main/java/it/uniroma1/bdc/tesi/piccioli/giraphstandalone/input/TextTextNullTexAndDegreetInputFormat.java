 ...
 */
public class TextTextNullTexAndDegreetInputFormat extends
    TextVertexInputFormat<TextPair, Text, NullWritable> {
...
//  private static final Pattern SEPARATOR = Pattern.compile("[\t ]");
  ...
  public class TextTextNullVertexReader extends
    ...
    protected String[] preprocessLine(Text line) throws IOException {
      ...
      id = new TextPair(new Text(tokens[0]),new Text("init"));
      value = new Text(Double.toString(0.0));
      return tokens;
    }

    @Override
    protected TextPair getId(String[] tokens) throws IOException {
      return id;
    }
    ...
    @Override
    protected Iterable<Edge<TextPair, NullWritable>> getEdges(
        String[] tokens) throws IOException {
      List<Edge<TextPair, NullWritable>> edges =
          Lists.newArrayListWithCapacity(tokens.length - 1);
      for (int n = 1; n < tokens.length; n++) {
        edges.add(EdgeFactory.create(
            new TextPair(new Text(tokens[0]),new Text())));
      }
      return edges;
    }
  }
